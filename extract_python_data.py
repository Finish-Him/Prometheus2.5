...
def extract_py_info(file_path):
    ...
    # Extrair comentários de documentação (docstrings)
    ...
    string_literals = re.findall(r'(?:\'\'\'([\s\S]*?)\'\'\'|"""([\s\S]*?)"""|\')((?:[^\']|\\\')*?)\'|"((?:[^"]|\\")*?)")', content)
    for i, literals in enumerate(string_literals):
        for literal in literals:
            if literal and len(literal.strip()) > 10:  # Ignorar strings muito curtas
                string_info = {
                    "type": "python_string",
                    "file": file_name,
                    "string_index": i,
                    "text": literal.strip()[:500]  # Limitar tamanho do texto
                }
                py_info.append(string_info)
                break  # Apenas uma das alternativas terá conteúdo
    
    # Extrair informações específicas com base no nome do arquivo
    if "api" in file_name.lower() or "collector" in file_name.lower():
    ...
    elif "analyze" in file_name.lower() or "predict" in file_name.lower():
        # Extrair variáveis importantes
        variables = re.findall(r'^([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*([^\n]+)', content, re.MULTILINE)
        for i, (name, value) in enumerate(variables):
            if not name.startswith('_') and len(value.strip()) > 0:  # Ignorar variáveis privadas
                try:
                    # Tentar avaliar o valor como expressão Python
                    eval_value = ast.literal_eval(value.strip())
                    var_info = {
                        "type": "python_variable",
                        "file": file_name,
                        "variable_index": i,
                        "name": name,
                        "value": str(eval_value)
                    }
                except:
                    # Se não for possível avaliar, armazenar como string
                    var_info = {
                        "type": "python_variable",
                        "file": file_name,
                        "variable_index": i,
                        "name": name,
                        "value": value.strip()[:100]  # Limitar tamanho do valor
                    }
                py_info.append(var_info)
